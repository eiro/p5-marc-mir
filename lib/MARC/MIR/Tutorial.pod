...
for the moment, everything is found in MARC::MIR namespace, this will change. Also t/* is empty for the moment (this is clearly the next step) the scripts that uses MARC::MIR are yet working.

=head2 What is MARC::MIR

I dealt with lot of MARC records in the past (mainly from/to iso2709 files) and was really borring by the existing libraries. A MARC record is a very simple structure. every library i saw missed this point by wrapping records into painfull libraries, this make the MARC manipulation anoying and slow. Perl is awesome for manipulate datastructures: i wanted those power and simplicity back!
...
A MIR control field is a tag, a MIR subfield_collection and an optionnal MIR indicator. The MIR indicator is a 2 char string or a 2 elements array. so all those MIR field_collection are valid.
...
every 856$q must be jpeg
	...
	$_ = from_iso2709;
	map_fields {
	    tag eq '856' and map_those_subfields {
